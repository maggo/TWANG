...
#define SPACING 1
int PIXELS_PER_ROW;
int DRAW_WIDTH = PIXEL_WIDTH + SPACING;
...
#define WIFI_USER "FRITZ!Box 7490"
#define WIFI_PASS "16870254233795588547"
#define WEBSOCKET_PORT 80

WiFiMulti Wifi;
WebSocketsServer WSServer = WebSocketsServer(WEBSOCKET_PORT);
...
void Player::setButton(bool state)
{
    button = state;
}

void Player::setJoystick(int x, int y)
{
    joystickX = x;
    joyStickY = y;
}
...
void setup()
{
    ...
    PIXELS_PER_ROW = Heltec.display->getWidth() / DRAW_WIDTH;
    ...
    // Wifi
    Wifi.addAP(WIFI_USER, WIFI_PASS);
...
void loop()
{
    ...
    if (mm - previousMillis >= MIN_REDRAW_INTERVAL)
    {
        ...
        else if (stage == "WIN")
        {
            ...
            {
                int n = _max(map(((mm - stageStartTime)), 500, 1000, NUM_LEDS, 0), 0);
                for (int i = 0; i < n; i++)
                {
                    brightness = 255;
                    leds[i] = CRGB(0, brightness, 0);
        ...
        drawDebugScreen(leds);

        FastLED.show();
    }
}

void drawDebugScreen(CRGB leds[])
{
    Heltec.display->clear();
    ...
    {
        CRGB led = leds[i];
        int row = floor(i / PIXELS_PER_ROW * DRAW_WIDTH);

        if (led == CRGB(CRGB::Black))
        {
            Heltec.display->drawRect(i % PIXELS_PER_ROW * DRAW_WIDTH, row, PIXEL_WIDTH, PIXEL_WIDTH);
        }
        else
        {
            Heltec.display->fillRect(i % PIXELS_PER_ROW * DRAW_WIDTH, row, PIXEL_WIDTH, PIXEL_WIDTH);
        }
    }
...
{
    char *rawstr = (char *)payload;
    char msgbuf[10];
    String str = String(rawstr);
    str.trim();
    IPAddress ip = WSServer.remoteIP(num);
    String msg = String("LIVES:") + lives;
    ...
    case WStype_TEXT:
        ...
        else if (str == "BUTTON_UP")
        {
            Serial.println("Button Up");
            player1.setButton(false);
        }

        if (str.startsWith("JOYSTICK:"))
        {
            Serial.println(str);
            strsep(&rawstr, ":");
            int x = strtol(strsep(&rawstr, ":"), NULL, 10);
            int y = strtol(strsep(&rawstr, ":"), NULL, 10);

            Serial.printf("X: %d, Y: %d\n", x, y);

            player1.setJoystick(x, y);
        }
    ...
    for (i = 0; i < conveyorCount; i++)
    {
        if (conveyorPool[i]._alive)
        {
            ...
            if (playerPosition > conveyorPool[i]._startPoint && playerPosition < conveyorPool[i]._endPoint)
            {
                if (dir == -1)
                {
                    playerPositionModifier = -(MAX_PLAYER_SPEED - 6);
                }
                else
                {
                    playerPositionModifier = (MAX_PLAYER_SPEED - 6);
...
void getInput()
{
    ...
    joystickTilt = player1.joystickX;
    joystickWobble = player1.button ? ATTACK_THRESHOLD + 1 : 0;
}
